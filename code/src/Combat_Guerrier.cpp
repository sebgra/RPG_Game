...
void Combat_Guerrier(Guerrier &Heros, Personnage &Monstre) // permet de declencher un combat entre deux personnages selon la mecanique si dessous

{
	...
	while(Heros.get_vie() > 0 && Monstre.get_vie()> 0) // le combat perdure tant que le joueur et le monstre ont de la vie
	{
		...
		{
			Heros.charge(Monstre, Heros); // Appel a la methode d'attaque de personnage
		}

		else if (choix_joueur == 1) // Si le joueur entre 1 --> le personnage est soigné
		{
			Heros.soin_Guerrier(); // Appel a la methode de soin du personnage
		}
