...
void Combat_Magicien(Magicien &Heros, Personnage &Monstre) // permet de declencher un combat entre deux personnages selon la mecanique si dessous

{
	...
	std::cout << "Mana de Heros"<<Heros.get_Mana()<< std::endl; // rappelle au joueur sa vie
	...
	while(Heros.get_vie() > 0 && Monstre.get_vie()> 0) // le combat perdure tant que le joueur et le monstre ont de la vie
	{
		...
		std::cout << "Que voulez vous faire ? 0: Attaquer , 1: Soin, 2: Regenerer le Mana"<< std::endl; // demande au joueur de definir l'action a effectuer
		...
		{
			Heros.BouleDeFeu(Monstre); // Appel a la methode d'attaque de personnage
		}

		else if (choix_joueur == 1) // Si le joueur entre 1 --> le personnage est soigné
		{
			Heros.soin_Magicien(); // Appel a la methode de soin du personnage
		}

		else if (choix_joueur == 2) // Si le joueur entre 1 --> le personnage est soigné
		{
			Heros.regen_mana(); // Appel a la methode de regeneration du mana
		}


		std::cout << "Vie de Heros"<<Heros.get_vie()<< std::endl; // rappel des statuts apres action du joueur
		std::cout << "Mana de Heros"<<Heros.get_Mana()<< std::endl;
