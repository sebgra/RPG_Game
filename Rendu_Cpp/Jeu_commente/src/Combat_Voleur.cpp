...
void Combat_Voleur(Voleur &Heros, Personnage &Monstre) // permet de declencher un combat entre deux personnages selon la mecanique si dessous

{
	...
	while(Heros.get_vie() > 0 && Monstre.get_vie()> 0) // le combat perdure tant que le joueur et le monstre ont de la vie
	{
		...
		{
			Heros.sournoiserie(Monstre); // Appel a la methode d'attaque de personnage
		}

		else if (choix_joueur == 1) // Si le joueur entre 1 --> le personnage est soigné
		{
			Heros.soin_Voleur(); // Appel a la methode de soin du personnage
		}
